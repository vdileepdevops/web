...
@Component({
  selector: 'app-referral-agent-master',
  templateUrl: './referral-agent-master.component.html',
  styles: []
})
export class ReferralAgentMasterComponent implements OnInit {
  ...
  public SaveHideandShow: boolean;
    public buttondata: any = [{ id: 1, data: "selectreferral", type: "Select Referral / Agent", clear: true, next: true, back: false, save: false }, { id: 2, data: "kyc-Documents", type: "KYC Documents", clear: true, next: true, back: true, save: false }, { id: 3, data: "bankdetails", type: "Bank Details", clear: true, next: true, back: true, save: false }, { id: 4, data: "taxes", type: "Taxes", clear: true, next: false, back: true, save: true }];
  public today: Date = new Date();
  public Referraldata: any;
  public button: any = 'Submit';
  public isLoading: boolean;

  constructor(private _referalagentmasterservice: ReferralAgentMasterService, private toastr: ToastrService, private _commonService: CommonService, private formbuilder: FormBuilder, private router: Router) { }
  ...
  ngOnInit() {
      ...
      this.selectedtype = "Select Referral / Agent";
  ...
  getReferralData() {
    ...
    this.Referraldata = [];
    if (Referralid != '') {
      this._referalagentmasterservice.getViewReferralDetails(Referralid).subscribe(json => {
  ...
  public ShowTitle(type) {
    ...
    this.clearselection = this.selectedtype.replace(/\s/g, "").replace("/", "");
    this._referalagentmasterservice.SetSelectTitileRefferalCreation(type);
    this.buttonfn();
  }
  ...
  public nextcomponent() {
      ...
      if (this.selectedtype == 'Select Referral / Agent') {
  ...
  Savereferralagent() {
    ...
    this._referalagentmasterservice.checkContactIndividual(contactid, RefId).subscribe(res => {
      if (res > 0) {
        count = 1;
        this.isLoading = false;
        this.button = 'Submit'
        ...
        this._referalagentmasterservice.savereferraldata(data).subscribe(res => {
          ...
          this.router.navigate(['/ReferralAgentView'])
          //this.ClearForm();
        },
          (error) => {
            this.isLoading = false;
