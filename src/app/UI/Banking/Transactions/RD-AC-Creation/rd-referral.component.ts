...
declare let $: any
@Component({
  selector: 'app-rd-referral',
  templateUrl: './rd-referral.component.html',
  styles: []
})
export class RdReferralComponent implements OnInit {
 
  constructor(private fiIndividualService: FIIndividualService, private _fb: FormBuilder, private _commonService: CommonService, private savingtranscationservice: SavingtranscationService, private zone: NgZone, private router: Router,private _rdtranscationservice:RdTransactionsService) { }
  referralAgents: any;
  ...
  RefferalApplicable() 
  {
    ...
    // this.referralForm.controls.pSalesPersonName.setValidators(Validators.required)
      ...
      this.ShowReferral = false;
      this.referralForm.controls.pReferralId.clearValidators()
      this.referralForm.controls.pReferralId.updateValueAndValidity()
     // this.referralForm.controls.pSalesPersonName.clearValidators()
  ...
  {
    debugger
     let commisiondata=this._rdtranscationservice._GetCommisiontype()
    debugger
    ...
    // }
    this._rdtranscationservice.SaveRDReferralData(data).subscribe(json=>{
      if(json)
      {
