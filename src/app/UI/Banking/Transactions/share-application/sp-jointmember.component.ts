...
declare let $: any
@Component({
  selector: 'app-sp-jointmember',
  templateUrl: './sp-jointmember.component.html',
  styleUrls: []
})
export class SpJointmemberComponent implements OnInit, DoCheck {
  @ViewChild(NomineedetailsComponent, { static: false }) nomineeDetails: NomineedetailsComponent
  gridsum: any;
  formatteddata: any;
  constructor(private _fb:FormBuilder,private _commonservice:CommonService,private savingtranscationservice:SavingtranscationService,private _MemberService:MemberService,private _fdrdtranscationservice:FdRdTransactionsService, private _ShareapplicationService:ShareapplicationService) { }
...
//             }
  ...
  {
    debugger
    let a= this._ShareapplicationService._GetCommisiontype();
    // this.JointMemberAndNomineeForm.controls.pAccountId.setValue(a['FdAccountId'])
    // this.JointMemberAndNomineeForm.controls.paccountNo.setValue(a['Fdaccountno'])
 ...
 this.contacttype=this._fdrdtranscationservice.GetBusinessEntityStatus()
  ...
  Save()
  {
    ...
    //    console.log(data)
    //    debugger
    //   this._ShareapplicationService.SaveJointMember(data).subscribe(json=>{
    //     if(json)
    //     {
    //       this.clear()
    //      let str = "refferral"
