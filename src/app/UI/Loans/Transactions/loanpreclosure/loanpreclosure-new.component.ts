...
@Component({
  selector: 'app-loanpreclosure-new',
  templateUrl: './loanpreclosure-new.component.html',
  styles: []
})
export class LoanpreclosureNewComponent implements OnInit {
  preClosureForm:FormGroup;
  // preClosureForm: FormGroup;
  loandropForm: FormGroup;
  pFormname:any = "PRECLOSURE"
  ...
  balnaceduevalidationforsavingtime:boolean;
  ...
  ngOnInit() {
    ...
    this.preClosureForm = this.formbuilder.group({
      pmodofreceipt: ['CASH'],
      pbankname: [''],
      pBranch: [''],
      pbranchname: [''],
    ...
    this.BlurEventAllControll(this.preClosureForm)
  ...
  typeofDepositBank(args) {
    this.GetValidationByControl(this.preClosureForm, 'pdepositbankid', true);
    let type = args.target.options[args.target.selectedIndex].text;

    this.preClosureForm.controls.pdepositbankname.setValue(type)

    this.getBankBranchName(this.preClosureForm.controls.pdepositbankid.value);
  }
  ...
  public Paymenttype(type) {
    ...
    this.preClosureForm.controls['pbankname'].setValue('');
    this.preClosureForm.controls['pChequenumber'].setValue('');
    this.preClosureForm.controls['pchequedate'].setValue(this.today);
    this.preClosureForm.controls['pdepositbankname'].setValue('');
    this.preClosureForm.controls['ptypeofpayment'].setValue('');
    this.preClosureForm.controls['pbranchname'].setValue('');
    this.preClosureForm.controls['pCardNumber'].setValue('');
    if (type == 'Bank') {
      this.preClosureForm.controls.ptranstype.setValue('Cheque');
      this.Banktype('Cheque')
      this.Modeofpayment = type;
    }
    else {
      this.preClosureForm.controls.ptranstype.setValue('');
      let DepositBankNameControl = this.preClosureForm.controls['pdepositbankname']
      let BankControl = this.preClosureForm.controls['pbankname'];
      let ChequeControl = this.preClosureForm.controls['pChequenumber']
      let TypeofPAymentControl = this.preClosureForm.controls['ptypeofpayment'];
      let BranchControl = this.preClosureForm.controls['pbranchname'];
      let CardNumberControl = this.preClosureForm.controls['pCardNumber'];
      let ChequeDateControl = this.preClosureForm.controls['pchequedate']
  ...
  public Banktype(type) {
    ...
    this.preClosureForm.controls.pdepositbankid.setValue('');
    this.preClosureForm.controls.pdepositbankname.setValue('');
    if (type == 'Online') {
      this.preClosureForm.controls.ptypeofpayment.setValue('');
      ...
      this.preClosureForm.controls.ptypeofpayment.setValue(type);
      if (type == 'Debit Card' || type == 'Credit Card') {

        let DepositBankDisable
        let Modeofpayment = this.preClosureForm.controls.pmodofreceipt.value.toUpperCase();
        ...
        const DepositBankIDControl = <FormGroup>this.preClosureForm['controls']['pdepositbankid'];
  ...
  validation(type) {
    ...
    let BankControl = this.preClosureForm.controls['pbankname']
    let CardNumberControl = this.preClosureForm.controls['pCardNumber']
    let DepositBankNameControl = this.preClosureForm.controls['pdepositbankid'];
    DepositBankNameControl.clearValidators();
  ...
  typeofPaymentChange(args) {
    this.GetValidationByControl(this.preClosureForm, 'ptypeofpayment', true);
    let type = args.target.options[args.target.selectedIndex].text;
    if (this.Transtype != '') {
      this.preClosureForm.controls.pdepositbankid.setValue('');
      this.preClosureForm.controls.pdepositbankname.setValue('');
  ...
  bankName_Change($event: any): void {
    ...
    this.upinameslist = []
    this.preClosureForm['controls']['pUpiname'].setValue('');
    this.preClosureForm['controls']['pUpiid'].setValue('');
    if (pbankid && pbankid != '') {
      ...
      this.preClosureForm['controls']['pbankname'].setValue(bankname);
    }
    else {
      this.preClosureForm['controls']['pbankname'].setValue('');
    }
  ...
  getBankBranchName(pbankid) {
    ...
    this.preClosureForm['controls']['pbranchname'].setValue(data[0].pbranchname);
...
getParticulars(){        
  this.loading = true;
  this._loaanreceiptServie.getParticularsById(this.loanreceiptAppliactionId, this.datePipe.transform(this.applicationdate, 'dd-MMM-yyyy').toString(),'Preclosure').subscribe((data: any) => {
    this.loading = false;
    ...
    if (data) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (data[i].pParticularsname == 'OUTSTANDING PRINCIPLE') {
         ...
         if (data[i].pParticularsname == 'OUTSTANDING INTEREST') {
          this.amount =  this.amount + data[i].pAmount;
         }
...
//<----Getting charge names(dropdown) (started)---->
  ...
  grandTotal() {
    ...
    for (let i = 0; i < this.dataOfParticulars.length; i++) {
      if(this.dataOfParticulars[i].pParticularsname == 'OUTSTANDING PRINCIPLE' || this.dataOfParticulars[i].pParticularsname == 'OUTSTANDING INTEREST'){
        this.dataOfParticulars[i].receipt = this.dataOfParticulars[i].pAmount
      }
...
//<----calculating grand totals of data of particulars (ended)------>
  ...
  calAmt(event, i, amountType) {
    ...
    if (this.dataOfParticulars[i].pParticularsname == 'OUTSTANDING PRINCIPLE' || this.dataOfParticulars[i].pParticularsname == 'OUTSTANDING INTEREST') {
      if ((this.dataOfParticulars[i].pParticularsname == 'OUTSTANDING PRINCIPLE' || this.dataOfParticulars[i].pParticularsname == 'OUTSTANDING INTEREST')  && ((this.dataOfParticulars[i].receipt ? Number((this.dataOfParticulars[i].receipt.toString()).replace(/,/g, "")) : 0) > (this.dataOfParticulars[i].pAmount ? Number((this.dataOfParticulars[i].pAmount.toString()).replace(/,/g, "")) : 0))) {
        this.dataOfParticulars[i].balance = this.dataOfParticulars[i].pAmount;

        this._commonService.showWarningMessage(this.dataOfParticulars[i].pParticularsname+" Amount Received should not be greater than Amount Due");
        ...
        for (let i = 0; i < this.dataOfParticulars.length; i++) {          
          this.emiAmount = this.emiAmount + (this.dataOfParticulars[i].receipt ? Number((this.dataOfParticulars[i].receipt.toString()).replace(/,/g, "")) : 0);
        }
        this.fixedemiAmount = this.emiAmount
      }
    }
 ...
 //<----function for selecting single charge (ended)---->
  ...
  receivedAmountcal() {
    ...
    for (let i = 0; i < this.dataOfParticulars.length; i++) {
      if (this.dataOfParticulars[i].pParticularstype == 'CHARGES' || this.dataOfParticulars[i].pParticularstype == 'PRECLOSURE CHARGES') {
      ...
      if (this.dataOfParticulars[i].pParticularsname == 'PENAL INTEREST' || this.dataOfParticulars[i].pParticularstype == 'PENAL INTEREST') {
...
//<-----saving data to api(started)---->
  saveLoanReceipt() {
    ...
    for (let i = 0; i < this.dataOfParticulars.length; i++) {
      ...
      if((this.dataOfParticulars[i].balance ? Number((this.dataOfParticulars[i].balance.toString()).replace(/,/g, "")) : 0) != 0){
          this.balnaceduevalidationforsavingtime = false;
          break;
      }
      else{
        this.balnaceduevalidationforsavingtime = true;

      }
    }
    if (this.checkValidations(this.preClosureForm, isvalid) && this.checkValidations(this.loandropForm, isvalid) && this.forNarrationErrorValid) {

      this.modeOfData = this.preClosureForm.value;
      ...
      let emiReceipt = null;  
       let noaddemireceipt = {
        ...
        pDetailsreceivabledate: new Date(this.preClosureForm.value.pDattransdate),
      ...
      let data = {
        ...
        pReceiptdate: new Date(this.preClosureForm.value.pDattransdate),
        ...
        pBank: this.preClosureForm.value.pbankname,
        pBankid: 0,
        pDeposibankid: this.preClosureForm.value.pdepositbankid ? this.preClosureForm.value.pdepositbankid : 0,
        pTranstype: this.preClosureForm.value.ptranstype,
        pBranch: this.preClosureForm.value.pbranchname,
        pChequeno: this.preClosureForm.value.pChequenumber,
        pTransdate: new Date(this.preClosureForm.value.pchequedate),
        pChrclearstatus: null,
        pNarration: this.narration,
        pTypeofpaymentonline: this.preClosureForm.value.ptypeofpayment ? this.preClosureForm.value.ptypeofpayment : "CASH",
        pUpiid: null,
        pVchcardissuebank: this.preClosureForm.value.pbankname,
        pVchcardnumber: this.preClosureForm.value.pCardNumber,
        pVchcardtype: null,
      ...
      if (this.preClosureForm.value.pmodofreceipt == 'BANK') {
        if (this.preClosureForm.value.ptypeofpayment == 'Cheque') {
          data.pChrclearstatus = "N";
          data.pTransdate = new Date(this.preClosureForm.value.pchequedate);

        }
        else {
          data.pChrclearstatus = "P";
          if(this.preClosureForm.value.ptypeofpayment == 'Credit Card'|| this.preClosureForm.value.ptypeofpayment == 'Debit Card' ){
            data.pTransdate =  new Date(this.preClosureForm.value.pDattransdate)
          }
        }
      }
      else {
      ...
      if (Number((this.forReceiptValidarion.toString()).replace(/,/g, "")) > 0 ) {
        if(this.balnaceduevalidationforsavingtime){
      ...
      console.log("data11111",data);
        ...
        this._loaanreceiptServie.saveLoanReceiptData(data).subscribe((data: any) => {
          if(data) {
            ...
            this.preClosureForm.value.loanapplication = '';
            $("#" + this.dynamicLoanapplicationId).data("kendoMultiColumnComboBox").dataSource = [];
            let receipt = btoa(data.pReceiptId + ',' + 'Loan Receipt');
          ...
          this._commonService.showWarningMessage("Balance Due amount should  be zero");
        }
      }
      else {
...
//<-----saving data to api(ended)---->
  ...
  BankNameChange() {
    this.GetValidationByControl(this.preClosureForm, 'pbankname', true);
  }
  ChequeNoChange() {
    this.GetValidationByControl(this.preClosureForm, 'pChequenumber', true);
  }
  ChequeDateChange() {
    this.GetValidationByControl(this.preClosureForm, 'pchequedate', true);
  }
  CardNoChange() {
    this.GetValidationByControl(this.preClosureForm, 'pCardNumber', true);
  }
  openModal() {
 ...
 clearData() {
    ...
    this.noaddbuttonforcharges = true;
    this.preClosureForm.controls.pmodofreceipt.setValue('CASH');
...
//<-----set min date based on requirement (started)---->
  setDateConfig(){
    if(this.pDateofapplication){
      ...
      this.preClosureForm.patchValue({
